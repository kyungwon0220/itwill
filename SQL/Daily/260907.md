...
- ' Subquery ' == SQL 문장 안에, 다른 SQL ( 소괄호로 묶어서 사용 )
- Subquery 먼저 수행하고, 그 결과를 Main(Outer) 쿼리절에서 사용
...
ORDER BY 3 DESC;
```
> Scalar 서브 쿼리 예제 코드 ( 하나의 행이 아닌, 하나의 Value 반환 (단일 컬럼, 단일값) )

| 두 코드는, 같다
- Query Execution Cache 기능 수행 ( 동일한 입력값이 들어오면, 캐시에 담아두고 재사용 )
- 입력값이 없는 데이터가 들어오면, NULL 반환 (OUTER JOIN 동일한 성격)
- GROUP BY 절에서를 제외하고 사용 가능
